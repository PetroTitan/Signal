...
/**
 * POST /api/identity/:identityId/devto/sign-out
 *
 * Identity-scoped sign-out for dev.to. Mirrors the Bluesky sign-out
 * pattern: revoke the encrypted API key + clear handle_mismatch
 * metadata + mirror growth_accounts.connection_status =
 * "not_connected". Idempotent; affects only THIS identity's row.
 *
 * No request body. The route is identity-keyed via the URL path.
 */
export async function POST(
  ...
  try {
    ...
    if (conn) {
      try {
        await markConnectionStatus({
          ...
          message: "Operator signed out of this dev.to account.",
          clearMetadataKeys: ["handle_mismatch"],
        });
      } catch (err) {
        console.error("[devto/sign-out] markConnectionStatus failed", err);
        return jsonError(
          500,
          "sign_out_failed",
      ...
      try {
        await recordActivity({
          ...
          entityId: conn.id,
          title: `Signed out of dev.to as ${conn.handle ?? "(unknown username)"}`,
          description: "Operator-triggered sign-out.",
        });
      } catch (err) {
        console.error("[devto/sign-out] activity log failed", err);
      }
    }
    ...
    try {
      ...
      console.error(
        "[devto/sign-out] growth_accounts mirror failed",
        err,
      );
    }
    ...
    console.error("[devto/sign-out] unexpected error", err);
