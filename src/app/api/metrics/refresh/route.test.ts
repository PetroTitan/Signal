...
/**
 * D.1G — the metrics refresh route shares the scheduler tick's
 * shared-secret gate. No secret → 503; missing/wrong bearer → 401.
 * An unauthorized cron call must never run a refresh.
 */
...
describe("GET /api/metrics/refresh — auth", () => {
  it("503 when SCHEDULER_TICK_TOKEN is unset", async () => {
  ...
  it("passes auth with the right token (503 only because no service-role key here)", async () => {
    process.env.SCHEDULER_TICK_TOKEN = "secret";
